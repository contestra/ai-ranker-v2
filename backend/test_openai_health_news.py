#!/usr/bin/env python3
"""Test OpenAI with health and wellness news query - both ungrounded and grounded."""
...
async def test_openai_health_news():
    ...
    print("=" * 80)
    print("TEST 1: OPENAI UNGROUNDED WITH ALS=DE")
    print("=" * 80)
    print(f"\nQuery: {query}")
    ...
    try:
        print("\nSending ungrounded request to OpenAI...")
        ...
        if response.metadata:
            print(f"\nKey Metadata:")
            for key in ['als_present', 'als_country', 'finish_reason', 'tool_call_count']:
    ...
    print("\n" + "=" * 80)
    print("TEST 2: OPENAI GROUNDED WITH ALS=DE")
    print("=" * 80)
    print(f"\nQuery: {query}")
    print(f"ALS: {als_context.country_code} ({als_context.locale})")
    print("-" * 80)
    ...
    try:
        print("\nSending grounded request to OpenAI...")
...
if __name__ == "__main__":
    asyncio.run(test_openai_health_news())