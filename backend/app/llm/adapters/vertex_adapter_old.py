...
def _make_google_search_tool():
    """Create GoogleSearch tool using google-genai SDK."""
...
class VertexAdapter:
    ...
    def __init__(self):
        ...
        self.project = os.getenv("VERTEX_PROJECT", os.getenv("GCP_PROJECT"))
        self.location = os.getenv("VERTEX_LOCATION", "europe-west4")
        
        if not self.project:
            raise ValueError(
                "VERTEX_PROJECT or GCP_PROJECT environment variable is required. "
                "Please set one of these variables to your GCP project ID."
            )
        
        # Initialize google-genai client (REQUIRED - no fallback)
        try:
    ...
    def _build_content_with_als(self, messages: List[Dict], als_block: str = None) -> tuple[str, List[Content]]:
        """
        Build genai Content objects from messages.
    ...
    async def _step1_grounded_genai(self, req: LLMRequest, contents: List[Content], 
        ...
        logger.debug(f"[WIRE_DEBUG] Vertex Step-1 grounded call with genai client")
        logger.debug(f"  Mode: {mode}")
        
        # Map Contestra modes to API modes
        # AUTO → "AUTO", REQUIRED → "ANY" (since "REQUIRED" is invalid)
        api_mode = "AUTO" if mode == "AUTO" else "ANY"
        
        try:
            # Use genai client to generate
            model_name = req.model.replace("publishers/google/models/", "")
            
            # Use synchronous API in thread pool
            from starlette.concurrency import run_in_threadpool
...
Respond ONLY with valid JSON that addresses the original request."""
    ...
    async def complete(self, req: LLMRequest, timeout: int = 60) -> LLMResponse:
        ...
        system_instruction, contents = self._build_content_with_als(req.messages, None)
        ...
        if needs_two_step:
            ...
            step1_resp, grounded_effective, tool_call_count = await self._step1_grounded_genai(
                req, contents, generation_config, timeout, 
