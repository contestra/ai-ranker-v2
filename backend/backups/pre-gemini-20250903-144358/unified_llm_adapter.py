    ...
    async def _emit_telemetry(
        ...
        try:
            ...
            meta_json = {
                ...
                'required_pass_reason': response.metadata.get('required_pass_reason') if hasattr(response, 'metadata') else None,  # NEW: Track why REQUIRED passed
            ...
            logger.info(
                ...
                "als_present=%s tool_count=%s response_api=%s region=%s required_pass_reason=%s",
                request.vendor, request.model, request.grounded, response.grounded_effective,
                meta_json['als_present'], meta_json['tool_call_count'],
                meta_json['response_api'], meta_json['region'], meta_json.get('required_pass_reason')
            )
